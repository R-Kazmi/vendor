...
const UpdateProduct = () => {
  ...
  const { data: product } = useGetProductByIdQuery({ id: 8717550027049 });
  ...
  useEffect(() => {
    if (product) {
      Object.keys(product.data.items[0]).forEach((key) => {
        if (key === "images") {
          setSelectedImages(() => {
            return product.data.items[0].images.map((image) => image.src);
          });
        }
        setValue(key, product.data.items[0][key]);
      });
    }
  }, [product]);
                ...
                <div className="overflow-hidden max-h-max w-full">
                  ...
                  <div className="max-w-full">
                    ...
                    <ReactQuill
                      ...
                      value={product?.data.items[0].body_html}
